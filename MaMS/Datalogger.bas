...
$regfile "m644pdef.dat"                                       'Dem Programm sagen was f�r ein Controller verwendet wird
...
Config 1wire = Porta.5                                      'Port f�r 1wire DS18B20
'Buzzer Alias Porta.7                                        'Port f�r Buzzer (Sound)
Config Timer1 = Pwm , Pwm = 8 , Compare A Pwm = Clear Up , Compare B Pwm = Clear Up , Prescale = 1       'Konfiguration f�r PWM
Compare1a = 255                                             'L�fter abschalten
Compare1b = 255                                             'L�fter abschalten
Dim Runde As Integer                                        'Rundenz�hler: Deklarieren der Variable (Bekanntmachen der Variable)
...
Adcalarmsub1 = 0
Adcalarmsub2 = 0
Einbefehl = 0                                               'RS232 Pr�fvariable: Variablen auf 0 Setzten
Befehl = ""                                                 'Befehl = keiner
Runde = 0                                                   'Rundenz�hler: Variablen auf 0 Setzten
...
'Startroutinen ausf�hren''''''''''''''''''''''''''''''''''''''''''''''''''''''''
'Sound Buzzer , 400 , 650                                    'BEEP
Print "Pr�fe System..."
Compare1a = 0                                               'L�fter durchstarten
Compare1b = 0                                               'L�fter durchstarten
'Waitms 4000                                                'L�fter durchstarten lassen
'Compare1a = 50                                             'L�fter abschalten
'Compare1b = 50                                             'L�fter abschalten
Gosub 1wirecheck
Gosub 1wire
'Gosub 1wireausgabe
...
'Print Einbefehl
'Print Befehl
 If Einbefehl = 1 Then                                      'Pr�fen ob RS232 Signal Aktiv
      ...
      Elseif Befehl = "1wire" Then                          'Wenn RS232 Befehl "1wire" ist
            Gosub 1wire
            'Gosub 1wireausgabe                              'Sub f�r 1Wire Ausgabe aufrufen
            Print "EOF"                                     'Ausgabe beenden
...
'            Print "EOF"                                     'Ausgabe beenden
      ...
      Elseif Befehl = "?" Then                              'Wenn RS232 Befehl "?" ist
            Print "Hilfsmen�:"                              'Ausgabe: Hilfsmen�
            Print "- alive                 Info, ob das Geraet noch lebt"       'Ausgabe: Hilfsmen�
            Print "- 1wire                 Temperatursensoren Abfrage"       'Ausgabe: Hilfsmen�
            Print "- pwmseta<wert>         L�ftersteuerung 1. Luefter"       'Ausgabe: Hilfsmen�
            Print "- pwmsetb<wert>         L�ftersteuerung 2. Luefter"       'Ausgabe: Hilfsmen�
            Print "- pwmreada               Ausgabe der gesetzten Werte der L�ftererung CHAN A"       'Ausgabe: Hilfsmen�
            Print "- pwmreadb               Ausgabe der gesetzten Werte der L�ftererung CHAN B"       'Ausgabe: Hilfsmen�
            Print "- dht11                 DHT11 Sensor"
'            Print "- backupsystem_on       einschalten des Backupsystems"       'Ausgabe: Hilfsmen�
'            Print "- backupsystem_off      ausschalten des Backupsystems"       'Ausgabe: Hilfsmen�
'            Print "- backupsystem_status   Status des Backupsystems"       'Ausgabe: Hilfsmen�
'            Print "- backupsystem_alarm    Zustand des Backupsystems"       'Ausgabe: Hilfsmen�
            Print "- portstatus            ADC/T�rstatus des Serverschranks"       'Ausgabe: Hiltsmen
            Print "- ?                     Hilfe"           'Ausgabe: Hilfsmen�
            Print "EOF"                                     'Ausgabe beenden
      Else                                                  'Wenn der Befehl nicht erkannt wurde
            ...
            Print "EOF"                                     'Ausgabe beenden
      End If
      Einbefehl = 0                                         'Befehlseingabepr�fung zur�cksetzen
      Befehl = ""                                           'Variable f�r Befehl zur�cksetzen
      Pwmwert = ""                                          'Variable f�r Pwmwert zur�cksetzen
      Pwmwert2 = 0                                          'Variable f�r Pwmwert zur�cksetzen
      Enable Urxc                                           'Interrupt f�r RS232 wieder Aktivieren
 End If
 Incr Runde                                                 'Variable Runde um 1 erh�hen
 If Einbefehl <> 1 Then
    If Runde => 1200 Then                                   'Wenn 1200 Runden erreicht sind
         'If Einbefehl <> 1 Then
            ...
            'Print "Runde: " ; Runde
         'End If
'         Gosub 1wire                                        'Sub f�r Messung aufrufen
         'Gosub 1wireausgabe

'         If Backupsystemstatus = 1 Then
'            Gosub Backupsystem                              'Sub f�r Temperaturpr�fung aufrufen
            'Print "CHECK"
           'Print "EOF"
'         End If
...
'         Compare1b = 0
'         Alarm = 1
'         Print "SYSTEM WARNING: Temperatur liegt bei " ; Wertarray(1) ; " und somit �ber " ; Emergencytemp ; " Grad!"
...
Anzahlsensoren = 0
 ...
 Else                                                       'Wenn keine Sensoren gefunden wurden dann
     ...
     ' Sound Buzzer , 400 , 750                              'BEEP
      ...
      Goto 1wirecheck                                       'Pr�fung wieder von vorne anfangen
 End If
Return
...
'Print "1wire read start"
 ...
 1wwrite &HCC                                               '�berspringe Rombefehl (Skip Rom)
 1wwrite &H44                                               'Messung durchf�hren und im Speicher/Register ablegen (Convert T)
 Waitms 800                                                 'Auf Messung warten
 Pos = 0                                                    'Variable auf 0 Setzen damit er bei 0 anf�ngt zu z�hlen
 Anzahlsensoren = 0
...
' Print "1wire after 1wirecount"
 ...
 'End If
 While Pos < Anzahlsensoren                                 'F�r jeden gefundenen Sensor einen Ablauf der nachfolgenden Befehle
      If Pos = 0 Then                                       'Pr�fen ob der Durchlauf der erste ist
            Adresse(1) = 1wsearchfirst()                    'Erste ID des 1wire Bus in Variable(array) einlesen
      Else                                                  'Pr�fen ob der Durchlauf nicht der erste ist
            Adresse(1) = 1wsearchnext()                     'N�chste ID des 1wire Bus in Variable(array) einlesen
      End If
      Incr Pos                                              'Positionsz�hler einen h�her setzen
      Id = ""
      For I = 1 To 8                                        'Schleife f�r die Adresse
           ' If Einbefehl <> 1 Then
      ...
      'Print
      1wreset                                               'Initialisierung Des Bausteins
      1wwrite &H55                                          'Sensor ausw�hlen (Match Rom command)
      1wwrite Adresse(1) , 8                                'Adresse zum ausw�hlen senden
      1wwrite &HBE                                          'Inhalt(Temperaturwert) aus dem Register auslesen
      For I = 1 To 9                                        'Schleife F�r den Temperaturwert
            Daten(i) = 1wread()                             'Daten in ein Array schreiben
      Next I
      1wreset                                               'Initialisierung des Bausteins
 If Daten(2) >= 248 Then                                    'Pr�fen ob Temperatur im Negativ Bereich liegt
...
Onrxd:
 ...
 Elseif Zeichen = 13 Then                                   'Wenn nur Zeichen = CR (Enter)
      'Print Befehl ; "-" ; Pwmwert
      Einbefehl = 1                                         'Variable Eingabepr�fung auf ein Setzen
